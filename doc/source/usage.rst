...
**OVS_DB_CONF**
    ...
    Default: $OVS_DB_CONF_DIR/conf.db

**OVS_DB_SOCKET**
    (Filesystem path) File path to OVS socket.

    Default: $OVS_DB_SOCKET_DIR/db.sock
...
**OVS_ALLOCATE_HUGEPAGES**
    ...
    If 'True' OpenVSwitch/DPDK will allocate hugepages of the default size for currently running Operating System (i.e.: 2kB).
...
**OVS_GIT_TAG**
    (git tag/branch/commitId) Indicates which tag, branch or commitId of Open vSwitch source to checkout before compiling.
