...
User you will use for puppet deployment has to be in sudoers file.

Disable requiretty
==================
In ovsdpdk puppet module we use sudo for performing some of operations,
for security reasons on some CentOS installations terminal is required for performing sudo
operations. Please check in /etc/sudoers and disable that row:

| #Defaults requiretty

otherwise ovsdpdk puppet deployment will fail to execute sudo commands.
...
=====================
Let's start with fresh OS install, please ensure that you have supported version of CentOS.
...
Some of services are inactive (disabled on boot), you can ignore them.
I also received some deprecation warnings, most likely due to disabled legacy openstack-keystone service.
But especially all neutron services and support services should be in active state.
