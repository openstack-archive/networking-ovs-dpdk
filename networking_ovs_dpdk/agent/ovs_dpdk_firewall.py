...
class OVSFirewallDriver(firewall.FirewallDriver):
        ...
        for rule in rules:
            ...
            if (port_range_min and port_range_max and
                ...
                if port_range_max > port_range_min:
                    flow[port_match] = self.port_rule_masking(
                        port_range_min,
                        port_range_max)
                else:
    ...
    def _gen_rules_port_min(self, port_min, top_bit):
        """
        Encode a port range range(port_min, (port_min | (top_bit - 1)) + 1)
        into a set of bit value/masks.
        """
        # Processing starts with setting up mask and top_bit variables to their
        # maximum. Top_bit has the form (1000000) with '1' pointing to the
        # register being processed, while mask has the form (0111111) with '1'
        # showing possible range to be covered.

        # With each rule generation cycle, mask and top_bit are bit shifted to
        # the right. When top_bit reaches 0 it means that last register was
        # processed.
        ...
        #     2). port & top_bit == 0
        #     This means that kth port bit is equal to '0'. By setting it to
        #     '1' and masking other (k-1) bits all ports in range
        #     [P, P + 2^(k-1)-1] are guaranteed to be covered.
        #     Let p_k be equal to port first (n-k) bits with rest set to 0.
        #     Then P = p_k | top_bit.

        # Correctness proof:
        # The remaining range to be encoded is calculated as follows:
        # R = [port_min, port_min | mask].
        # If condition 1 holds, then a rule that covers R is generated and the
        # job is done.
        # If condition 2 holds, then the rule emitted will cover 2^(k-1) values
        # from the range. Remaining range R will shrink by 2^(k-1).
        # If condition 2 doesn't hold, then even after top_bit/mask shift in
        # next iteration the value of R won't change.
        ...
        while True:
            if (port_min & mask) == 0:
                ...
                rules.append(self._hex_format(port_min, mask))
                break
            top_bit >>= 1
            mask >>= 1
            if (port_min & top_bit) == 0:
                # matched next '0' in port_min to substitute for '1' in
                # resulting rule
                rules.append(self._hex_format(port_min & ~mask | top_bit,
                                              mask))
        return rules

    def _gen_rules_port_max(self, port_max, top_bit):
        """
        ...
        #     This means that kth port bit is equal to '1'. By setting it to
        #     '0' and masking other (k-1) bits all ports in range
        #     [P, P + 2^(k-1)-1] are guaranteed to be covered.
        #     Let p_k be equal to port first (n-k) bits with rest set to 0.
        #     Then P = p_k | ~top_bit.
        ...
        while True:
            if (port_max & mask) == mask:
                ...
                rules.append(self._hex_format(port_max & ~mask, mask))
                break
            top_bit >>= 1
            mask >>= 1
            if (port_max & top_bit) == top_bit:
                # matched next '1' in port_max to substitute for '0' in
                # resulting rule
                rules.append(self._hex_format(port_max & ~mask & ~top_bit,
                                              mask))
        return rules

    def port_rule_masking(self, port_min, port_max):
        """Translate a range [port_min, port_max] into a set of bitwise matches

        Each match has the form 'port/mask'. The port and mask are 16-bit
        numbers written in hexadecimal prefixed by 0x. Each 1-bit in mask
        requires that the corresponding bit in port must match. Each 0-bit in
        ...
        # covering range [0, 2^3), which equals to a single 010xxx rule.
        # The algo will return [0x0010/fff8].

        # Else [port_min, port_max] range will be split into 2:
        # range [port_min, T) and [T, port_max]. Let p_m be the common part of
        # port_min and port_max with other (n-m) bits set to 0.
        # Then T = p_m | 1 << (n-m-1).
        # For example:
        # n = 7
        ...
        # Each of the ranges will be processed separately, then the generated
        ...
        if bitdiff == 0:
            # port_min == port_max
            return [self._hex_format(port_min)]
        ...
        if (port_min & (top_bit - 1) == 0 and
            ...
            return [self._hex_format(port_min, top_bit - 1)]

        top_bit >>= 1
        rules = []
        rules.extend(self._gen_rules_port_min(port_min, top_bit))
        rules.extend(self._gen_rules_port_max(port_max, top_bit))
        return rules
