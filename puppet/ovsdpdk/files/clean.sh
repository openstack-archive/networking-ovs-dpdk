...
if [ $res -eq 2 ]; then
    echo "[warning] service pvs-dpdk is already stopped"
else
...
# Remove mech driver
echo "[debug] removing mech driver"
sudo rm -f /usr/bin/networking-ovs-dpdk-agent
...
# remove git repos
echo "[warning] for security reasons, git repos were not automatically removed, you can remove it once you're finished with them"
