...
class ovsdpdk::install_ovs_dpdk (
  ...
  file {'/etc/default/ovs-dpdk': content => template("${plugin_dir}/files/ovs-dpdk-conf"), mode => '0644' }
  ...
  # install mech driver
  exec {'install mech driver': command => "cd ${networking_ovs_dpdk_dir}; python setup.py install", path => ['/usr/bin','/bin'], user => root }
}
